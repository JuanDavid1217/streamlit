...
#--- PAGE CONFIG ---#
st.set_page_config(page_title="Socialize your Knowledge", ##Estas dos lineas
                   page_icon=":busts_in_silhouette:")      #Modifican el folder (pagina)
...
performance_score= st.sidebar.multiselect("Select the Performance Score",
                                          options=df['performance_score'].unique(), #Optiene las opciones
                                          default=df['performance_score'].unique()) #Las carga por defecto
st.sidebar.markdown("##")
...
#El siguiente query dice:
#Convierte las variables en columnas 
...
[['average_work_hours']].sort_values(by="average_work_hours"))#sort_values es para ordenar.
